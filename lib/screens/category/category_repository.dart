...
Future<List<CategoryData>> getCategoryList({int? categoryId, bool isStoreCached = false, int? page = 1, var perPage = PER_PAGE_ITEM, required List<CategoryData> list, Function(bool)? lastPageCallBack}) async {
  try {
    ...
    if (categoryId != null) {
        res = CategoryResponse.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoints.categoryList}?category_id=$categoryId&per_page=$perPage&page=$page', method: HttpMethodType.GET)));
    } else {
