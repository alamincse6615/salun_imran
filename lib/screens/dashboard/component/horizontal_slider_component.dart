            ...
            itemBuilder: (_, i) {
              ...
              return CachedImageWidget(url: data.sliderImage.validate(), height: 200, width: context.width(), fit: BoxFit.cover, radius: defaultRadius).onTap(() {
                if (data.type == SLIDER_TYPE_CATEGORY) {
                  ViewAllServiceScreen(categoryData: CategoryData(id: data.linkId,name: data.name,categoryImage: data.sliderImage),serviceTitle: data.name.validate(), categoryId: data.linkId).launch(context);
                } else if (data.type == SLIDER_TYPE_SERVICE) {
                  ViewAllServiceScreen(categoryData: CategoryData(id: data.linkId,name: data.name,categoryImage: data.sliderImage),serviceTitle: data.name.validate()).launch(context);
                }
