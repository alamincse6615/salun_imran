...
List<PaymentData> getPaymentList({bool includeCash = true}) {
  ...
  if (includeCash) {
    list.add(PaymentData(id: PaymentMethods.PAYMENT_METHOD_CASH, icon: ic_payment, paymentMethod: locale.cashAfterService));
  }
  ...
  if (appConfigurationResponseCached != null) {
    ...
    if (appConfigurationResponseCached!.airtelMoneyPay != null) {
      list.add(PaymentData(id: PaymentMethods.AIRTEL_MONEY, icon: ic_airtel_money_logo, paymentMethod: locale.airtelMoneyPayment));
    }
    if (appConfigurationResponseCached!.cinetPay != null) {
      list.add(PaymentData(id: PaymentMethods.CINET, icon: ic_cinet_pay, paymentMethod: locale.ciNetPay));
    }
    if (appConfigurationResponseCached!.sadadPay != null) {
      list.add(PaymentData(id: PaymentMethods.PAYMENT_METHOD_SADAD_PAYMENT, icon: ic_sadad_payment, paymentMethod: locale.sadadPayment));
    }
    if (appConfigurationResponseCached!.midtransPay != null) {
      list.add(PaymentData(id: PaymentMethods.PAYMENT_METHOD_MIDTRANS, icon: ic_midtrance_pay, paymentMethod: locale.midTransPayment));
    }
    if (appConfigurationResponseCached!.phonePayPay != null) {
      list.add(PaymentData(id: PaymentMethods.PAYMENT_METHOD_PHONEPE, icon: ic_phone_pe, paymentMethod: locale.phonePe));
    }
  }
